...
using EloBuddy.SDK;
using SharpDX;
using Settings = Kalista_Beta_Fixed.Menus.Misc.Sentinel;

namespace Kalista_Beta_Fixed
{
    public static class GameProcessoIntGames
    {
        ...
        private static void OnTick(EventArgs args)
        {
            ...
            if (Settings.Enabled && SpellsManager.W.IsReady() && Player.Instance.ManaPercent >= Settings.Mana && !Player.Instance.IsRecalling())
            {
                if (!Settings.NoModeOnly || Orbwalker.ActiveModesFlags == Orbwalker.ActiveModes.None)
                {
                    if (OpenLocations.Count > 0 && SentLocation == null)
                    {
                        var closestLocation = OpenLocations.Where(o => Locations[o.Item1][o.Item2].IsInRange(Player.Instance, SpellsManager.W.Range - MaxRandomRadius / 2))
                        ...
                        if (closestLocation != null)
                        {
                            ...
                            SpellsManager.W.Cast(randomized);
